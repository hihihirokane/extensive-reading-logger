...
タブ幅に納まる様に記述を圧縮してある。```169@Nov``` は11月のある日に読んだ結果、169 WPM だったことを表す。
```n/a@'15```は2015年に読み、しかも時間を測らなかったのでWPMが計算できないことを表す。今月に読んだものは、
たとえば```130@22 ```のように月の日だけ(22日)が表示される。プログラム
```./mktable.awk s```
を呼び出した日の月が「今月」であり、使用例での「今月」は12月である。
