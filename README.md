...
### 記録
各リーダーの読書時間とページ数を入力すると、後に**表示**する際に読書速度 (WPM) を計算してくれる。2万語を超えるリーダーについて最後までの読書時間を測るのは現実的ではないので、数ページを選んでその時間を記入するだけでも計算は可能である。
* 最後までの時間を測った場合 ```./readdone.awk o[xford] farrar w[hole] [0h]200m[0s] 88 --commit```
* ある5ページだけ読むのにかかる時間を測った場合```./readdone.awk o[xford] farrar 5 [0h]10m[0s] 88 --commit```
