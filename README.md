...
コマンド
```grep, cat```
による結果と同様であるが、このスクリプト群を利用すると使用者の手順を直接コマンド操作よりもっと簡潔にできる利点がある上、さらに累計語数や
WPM
の計算、長い一冊を分割して記録するといったこともできる。グレイデッド・リーダーではない読書の記録にも対応し、その語数も任意で記録するかどうかを決められるが、その場合の語数については
...
リスト
(.tsv
ファイル, このフォーマットはGoogle Spreadsheetsのエクスポート機能として用意されている)
...
## 構成物
以下、「```read.done``` が必要」と書いてあるものは読書リストが必要で、その既定の名前
```read.done```
だという意味である。
...
1. ```read.done``` 単なる実施例だが、これを用いて一通りの動作を確認できる。私自身のバックアップを兼ねてもいる。

## 使用方法
以下に使用方法を説明する。大きくわけて
1. 検索 ```./readdone.awk```
1. 記録 ```./readdone.awk --commit```
1. 表示 ```./mktable.awk```
の三つがある。
記録についてはただ必要項目を記録するだけでなく、「どのように読んだか」についても記録できる方法を提供している。
表示については非要約モードと要約モード、さらに非要約モードより冗長な饒舌モード、デバッグモードといった挙動を提供してある。
以下に、順を追って説明する。
...
音声の速度を第6引数として指定できる。音読やシャドウイングを語数に数えるかどうかは各自の自由にすればよいが、第6引数で指定した文字列がそのまま
```read.done```
に載る。
...
* ```./mktable.awk``` WPMが色つきで表示される。速さとその意味とその色との関係は以下の通りであり、速さの意味は『快読100万語』[1]におおよそ倣った。
...
1冊を分割して記録た場合については、どの1冊も1つのレコードで表示するのが既定の動作であるが、
```
$ ./mktable.awk
                                        .
                                        .
...
を指定した
(```awk, gawk```
それ自体へのオプションと混同しないように第1引数には
```--```
が必要で、このスクリプトへのオプションは第2引数以降で指定する)
場合には分割した通りにレコードを表示する。
